/**
 * Shared client-side PDF download utility for browser environments.
...
export async function downloadReportPdf(
  ...
  fallbackFilename = 'SITE_WORK_Report.pdf'
): Promise<void> {
  if (!payload.siteId) {
  ...
  if (!res.ok) {
    ...
    try {
      ...
      else if (res.status >= 500) errorMsg = 'PDF export failed on the server. Please try again.';
    }
